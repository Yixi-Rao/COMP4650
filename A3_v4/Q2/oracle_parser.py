...
def parse_gold(dp, gold):
    ...
    #! implement the parse gold function by modifying 
    ...
    while not dp.ctx.is_buffer_empty():
        V_i = dp.ctx.stack_top()
        V_j = dp.ctx.next_buffer_word_id()
        
        if (V_j, V_i) in gold:
            features = extract_features(dp.ctx)
            if dp.left_arc():
        ...
        reduce_ = False
        for k in range(V_i):
            if (k, V_j) in gold or (V_j, k) in gold:
                features = extract_features(dp.ctx)
                if dp.reduce():
                    states.append(features)
