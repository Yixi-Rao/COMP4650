...
def get_doc_to_norm(index, doc_freq, num_docs):
    ...
    #? calculate square of norm for all docs by using the tf-idf value
    for term in index.keys():
        for doc_id, doc_tf in index[term]:
            doc_norm[doc_id] += (doc_tf * np.log(num_docs / (1 + doc_freq[term])))**2
...
def run_query(query_token_counts, index, doc_freq, doc_norm, num_docs):
    ...
    for query_term, query_tf in query_token_counts:
        if query_term not in doc_freq:
            continue
        query_norm += (query_tf * np.log(num_docs / (1 + doc_freq[query_term])))**2
    ...
    for query_term, query_tf in query_token_counts:
        ...
        for doc_id, doc_tf in index[query_term]:
            tfidf_q = query_tf * np.log(num_docs / (1 + doc_freq[query_term]))
            tfidf_d = doc_tf * np.log(num_docs / (1 + doc_freq[query_term]))
            doc_to_score[doc_id] += tfidf_q * tfidf_d / (doc_norm[doc_id] * query_norm)
